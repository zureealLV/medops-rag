...
| Credential verification exhaustion | repeated invalid keys force scrypt work | key-prefix lookup before scrypt, bounded credential format, gateway-required deployment boundary | `tests/test_authentication.py` | no in-process/distributed rate limiter; public deployments require gateway abuse controls |
...
| Poison ingestion job | permanently unsupported or malformed input | stable parser failures become terminal without retry; transient worker exceptions are bounded and visible | `tests/test_ingestion_jobs.py` | Compose bounds the worker container, not each parser call; higher assurance needs per-job sandboxing |
...
| Metrics privacy leak | operator reads another tenant's operational facts or raw content | admin authorization, tenant predicate and aggregate-only response without queries/document text/secrets | `tests/test_observability_metrics.py` | high-cardinality timing patterns can still reveal workload shape |
| Migration or rollback data loss | partial schema upgrade or invalid backup | pre-migration SQLite backup, integrity check, explicit schema version and tested full-file restore | `tests/test_v1_upgrade_rollback.py` | restoring a backup discards legitimate writes made after that backup |
| Container breakout/host impact | compromised parser or dependency | non-root user, read-only root, dropped capabilities, no-new-privileges, PID cap and bounded tmpfs | `compose.yaml`, `scripts/compose_smoke.py` | Docker kernel boundary is not a per-document micro-VM; no network egress policy |
